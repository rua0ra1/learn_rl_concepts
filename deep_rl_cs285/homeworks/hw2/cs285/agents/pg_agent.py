...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        # beyond this point.
        # HINT: use np.concatenate to flatten the lists of arrays into single arrays
        ...
        # step 2: calculate advantages from Q value TODO: needs to be
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = []
            for reward_traj in rewards:
                q_val_traj = self._discounted_return(reward_traj.tolist())
                q_values.append(np.array(q_val_traj))
        else:
            ...
            q_values = []
            for reward_traj in rewards:
                q_val_traj = self._discounted_reward_to_go(
                    reward_traj.tolist())
                q_values.append(np.array(q_val_traj))
